...
const createSmallCards = (data) => {
    ...
    <div class="sm-product">
        ...
        <p class="sm-price" data-price="${data.sellPrice}">${data.sellPrice * data.item} bdt</p>
...
const updateBill = () => {
    ...
    billPrice.innerHTML = `${totalBill} bdt`;
}
    ...
    counts.forEach((item, i) => {
        ...
        counterMinus[i].addEventListener('click', () => {
            if (item.innerHTML > 1) {
                item.innerHTML--;
                totalBill -= cost;
                price[i].innerHTML = `${item.innerHTML * cost} bdt`;
